...
const getPendingUserActionsStatus = async (req, res) => {
    try {
        if (!req.user) {
            res.status(401).json({ message: 'Unauthorized' });
            return;
        ...
        (0, handle_utils_1.handleError)(res, error, 'Get pending user actions status');
    }
};
