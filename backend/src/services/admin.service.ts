import {
  ...
  PlaylistType,
  LabelRegistrationRequest, // Added for label registration
  RequestStatus, // Added for label registration
} from "@prisma/client";
...
export const getArtistRequestDetail = async (id: string) => {
  // Attempt 1: Find by ArtistProfile ID (original behavior for old requests)
  ...
  if (!requestData) {
    const artistRoleRequest = await prisma.artistRequest.findUnique({
      ...
      select: {
        ...
        portfolioLinks: true,       // Added portfolioLinks
...
export const approveArtistRequest = async (adminUserId: string, artistRequestId: string) => {
  const artistRequest = await prisma.artistRequest.findUnique({
    ...
    select: {
      ...
      portfolioLinks: true,   // Assuming this is Prisma.JsonValue
      requestedGenres: true,  // Assuming this is string[]
      requestedLabelName: true,
      status: true,
      user: { select: { id: true, email: true, name: true, username: true } }, // For notification
    },
  });
