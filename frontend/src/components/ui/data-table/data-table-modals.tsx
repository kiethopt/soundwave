...
export function EditAlbumModal({
  ...
  return (
    <Dialog open={!!album} onOpenChange={onClose}>
      <DialogContent
        className={`${theme === 'dark' ? 'bg-[#2a2a2a] border-[#404040]' : 'bg-white'} p-6 rounded-lg shadow-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto`}
      >
        <DialogHeader>
          <DialogTitle
            className={`text-xl font-semibold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mt-4">
          ...
          <div className="space-y-2">
            ...
            <div className="w-full flex flex-col items-center mb-4" onClick={handleCoverClick}>
              <div
                className={`w-40 h-40 rounded-md overflow-hidden cursor-pointer border-2 ${theme === 'dark' ? 'border-gray-600' : 'border-gray-300'} hover:opacity-90 transition-opacity relative`}
              >
                <img
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              required
              className={`w-full px-3 py-2 rounded-md border ${theme === 'dark' ? 'bg-[#3a3a3a] border-[#505050] text-white focus:border-white/50' : 'bg-white border-gray-300 text-gray-900 focus:border-gray-400'} transition-colors focus:outline-none`}
            />
          </div>
