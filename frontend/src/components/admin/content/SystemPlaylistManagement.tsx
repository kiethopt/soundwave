...
import { MoreHorizontal, Trash2, Search, Edit, Eye, Plus, ArrowUpDown, ArrowUp, ArrowDown, RefreshCw, Calendar } from 'lucide-react';
...
interface SortConfig {
  key: keyof Omit<Playlist, 'tracks' | 'totalTracks' | 'totalDuration' | 'canEdit' | 'basedOnMood' | 'basedOnGenre' | 'basedOnArtist' | 'basedOnSongLength' | 'basedOnReleaseTime' | 'trackCount'> | 'isAIGenerated' | 'type' | null;
  direction: 'asc' | 'desc';
}

export const SystemPlaylistManagement: React.FC<{ theme: 'light' | 'dark' }> = ({ theme }) => {
  ...
  const fetchPlaylists = useCallback(async (page: number, search: string, sort: SortConfig) => {
    setLoading(true);
    try {
      ...
      const response = await api.playlists.getAllBaseSystemPlaylists(token, page, limit, params.toString());

        if (response.success && response.data && response.pagination) {
        setPlaylists(response.data || []);
        setTotalPages(response.pagination?.totalPages || 1);
        } else {
        console.error('Unexpected API response structure for playlists:', response);
        const message = response.message || 'Failed to parse playlist data from API.';
          toast.error(message);
        setPlaylists([]);
        setTotalPages(1);
      }
  ...
  const handleBulkDeleteClick = () => {
    ...
    if (!window.confirm(`Are you sure you want to delete ${selectedPlaylistIds.size} selected playlist(s)? This action cannot be undone.`)) {
      return;
    }
    ...
    const deleteMultiplePlaylists = async () => {
      ...
      try {
        ...
        await Promise.all(
          Array.from(selectedPlaylistIds).map(id => api.admin.deleteSystemPlaylist(id, token))
        );
        
        toast.success(`Successfully deleted ${selectedPlaylistIds.size} playlist(s)`);
        refreshPlaylists();
        setSelectedPlaylistIds(new Set());
      } catch (err: any) {
  ...
  const isAllSelected = playlists.length > 0 && selectedPlaylistIds.size === playlists.length;
  const isIndeterminate = selectedPlaylistIds.size > 0 && selectedPlaylistIds.size < playlists.length;
                  ...
                  <tbody>
                    {sortedPlaylists.length > 0 ? (
                      sortedPlaylists.map((playlist) => (
                        <tr
                          key={playlist.id}
                          className={`border-b cursor-pointer ${theme === 'dark' ? 'bg-gray-800 border-gray-700 hover:bg-gray-600' : 'bg-white border-gray-200 hover:bg-gray-50'} ${selectedPlaylistIds.has(playlist.id) ? (theme === 'dark' ? 'bg-gray-700/50' : 'bg-blue-50') : ''} ${actionLoading === playlist.id ? 'opacity-50 pointer-events-none' : ''}`}
                        >
                          ...
                          <td className="py-4 px-6">
                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${playlist.type === 'SYSTEM' 
                          ...
                          <td className="py-4 px-6">
                            {playlist.isAIGenerated ? (
                              <span className={`px-2 py-1 rounded-full text-xs font-medium ${theme === 'dark' ? 'bg-green-900 text-green-300' : 'bg-green-100 text-green-800'}`}>Yes</span>
                            ) : (
                              <span className={`px-2 py-1 rounded-full text-xs font-medium ${theme === 'dark' ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-800'}`}>No</span>
                            )}
                          </td>
                          <td className="py-4 px-6">{playlist.totalTracks ?? 0} tracks</td>
                          <td className="py-4 px-6">{formatPlaylistDate(playlist.createdAt)}</td>
                          <td className="py-4 px-6">
                      ...
                      <tr>
                        <td colSpan={7} className="py-4 px-6 text-center">No playlists found {activeSearchTerm ? 'matching your search' : ''}.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
